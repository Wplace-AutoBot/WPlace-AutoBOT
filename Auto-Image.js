; (async () => {
  ...
  .resize-canvas-stack { position: relative; transform-origin: center center; display: inline-block; }
    ...
    async function createWasmToken(regionX, regionY, payload) {
        try {
            // Dynamically find the Pawtect module
            const scripts = Array.from(document.scripts);
            const mainScript = scripts.find(s => s.src.includes('/_app/immutable/entry/start.'));
            if (!mainScript) {
                console.error('❌ Main app script not found');
                return null;
            }

            const appContent = await fetch(mainScript.src).then(res => res.text());
            const pawtectModulePathMatch = appContent.match(/"..\/chunks\/(BBb1ALhY\.[a-zA-Z0-9]+)\.js"/);

            if (!pawtectModulePathMatch || !pawtectModulePathMatch[1]) {
                console.error('❌ Pawtect module path not found in main script');
                return null;
            }

            const pawtectModulePath = `/_app/immutable/chunks/${pawtectModulePathMatch[1]}.js`;
            console.log('✅ Pawtect module path found:', pawtectModulePath);

            // Load the Pawtect module and WASM
            const mod = await import(pawtectModulePath);
            let wasm;
            try {
                wasm = await mod._();
