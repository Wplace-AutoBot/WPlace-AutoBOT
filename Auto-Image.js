...
(async () => {
    // ========================================
    // ENHANCED PAWTECT + WASM INTEGRATION
    // ========================================
    
    // Advanced WASM token generation from script.js
    async function createWasmToken(regionX, regionY, payload) {
        try {
            ...
            console.log('🎉 WASM TOKEN GENERATION SUCCESS!');
            console.log('🔑 Token length:', token?.length || 0);
            console.log('🔑 Token preview:', token?.substring(0, 50) + '...');
            return token;
        } catch (error) {
            console.error('❌ Failed to generate WASM token:', error);
            return null;
        }
    }

    // Generate random string for fingerprint
    ...
    // ========================================
    // ENHANCED TURNSTILE SOLVER (From V2)
    // ========================================
    ...
    function detectSitekey(fallback = '0x4AAAAAABpqJe8FO0N84q0F') {
        ...
        try {
            ...
            if (turnstileEl?.dataset?.sitekey && turnstileEl.dataset.sitekey.length > 10) {
                ...
                console.log("🔍 Sitekey detected from turnstile element:", _cachedSitekey);
                return _cachedSitekey;
            }
            ...
            if (typeof window !== 'undefined' && window.__TURNSTILE_SITEKEY && window.__TURNSTILE_SITEKEY.length > 10) {
                ...
                console.log("🔍 Sitekey detected from global variable:", _cachedSitekey);
                return _cachedSitekey;
            }
            ...
            for (const script of scripts) {
                ...
                if (sitekeyMatch && sitekeyMatch[1] && sitekeyMatch[1].length > 10) {
                    ...
                    console.log("🔍 Sitekey detected from script content:", _cachedSitekey);
                    return _cachedSitekey;
                }
            }
        } catch (error) {
            console.log('Error detecting sitekey:', error);
        }
    ...
    async function handleCaptcha() {
        ...
        try {
            ...
            const duration = Math.round(Date.now() - startTime);
            console.log(`❌ Turnstile token generation failed after ${duration}ms:`, error);
            throw error;
        }
    }
    ...
    async function executeTurnstile(sitekey, action = 'paint') {
        ...
        if (_turnstileWidgetId && _lastSitekey === sitekey && window.turnstile?.execute) {
            try {
                ...
                console.log('🔄 Widget reuse failed, will create a fresh widget:', err.message);
            }
        }
        ...
        let attempt = 1;
        while (true) {
            const currentTimeout = attempt === 1 ? 15000 : 5000;
            console.log(`🔄 Intento ${attempt} de resolución del CAPTCHA (timeout: ${currentTimeout/1000}s)...`);
            ...
            try {
                ...
                if (token && token.length > 20) {
                    console.log(`✅ CAPTCHA resuelto exitosamente en el intento ${attempt}`);
                    return token;
                }
                
                console.log(`⚠️ Intento ${attempt} falló, reintentando en 5 segundos...`);
                await sleep(5000);
    ...
    async function createNewTurnstileWidgetInvisible(sitekey, action) {
        return new Promise((resolve) => {
            try {
                ...
                console.log('Invisible Turnstile failed:', e);
                resolve(null);
            }
        });
    }
    ...
    async function createNewTurnstileWidgetInteractiveWithRetry(sitekey, action, isAutoRetry = true, customTimeout = 30000) {
        return new Promise((resolve, reject) => {
            try {
                ...
                const timeout = isAutoRetry ? customTimeout : 120000;
                const timeoutId = setTimeout(() => {
                    console.log(`⏰ Interactive Turnstile timed out (${isAutoRetry ? 'auto-retry' : 'manual'})`);
                ...
                const widgetId = window.turnstile.render(host, {
                    ...
                    size: 'normal',
                    retry: 'auto',
                    'retry-interval': isAutoRetry ? 3000 : 8000,
                    callback: (token) => {
                        clearTimeout(timeoutId);
                        try { overlay.remove(); } catch { }
                        console.log(`✅ Interactive Turnstile solved (${isAutoRetry ? 'auto-retry' : 'manual'})`);
                        resolve(token);
                    },
                    'error-callback': (error) => {
                        console.log(`🚨 Interactive Turnstile error (${isAutoRetry ? 'auto-retry' : 'manual'}):`, error);
                        if (isAutoRetry) {
                            clearTimeout(timeoutId);
                            try { overlay.remove(); } catch { }
                            resolve(null);
                    ...
                    'timeout-callback': () => {
                        console.log(`⏰ Turnstile timeout callback (${isAutoRetry ? 'auto-retry' : 'manual'})`);
                        if (isAutoRetry) {
                            clearTimeout(timeoutId);
                            try { overlay.remove(); } catch { }
                            resolve(null);
                    ...
                    'expired-callback': () => {
                        console.log(`⚠️ Interactive Turnstile token expired (${isAutoRetry ? 'auto-retry' : 'manual'})`);
                        if (isAutoRetry) {
                            clearTimeout(timeoutId);
                            try { overlay.remove(); } catch { }
                            resolve(null);
                ...
                console.log(`❌ Error creating interactive Turnstile widget (${isAutoRetry ? 'auto-retry' : 'manual'}):`, error);
                reject(error);
            }
        });
    }
    ...
    async function handleCaptchaFallback() {
        return new Promise((resolve, reject) => {
            const executeFlow = async () => {
                try {
                    console.log("🎯 Starting automatic CAPTCHA solving process...");
                    ...
                    const solvePromise = (async () => {
                        ...
                        if (!mainPaintBtn) {
                            console.log("🎯 No paint button found, clicking on canvas directly to trigger CAPTCHA...");
                        ...
                        console.log("🎯 Found paint button, clicking...");
                        mainPaintBtn.click();
                        await sleep(500);

                        console.log("🎯 Selecting transparent color...");
                        const transBtn = await waitForSelector('button#color-0', 200, 5000);
                        if (!transBtn) {
                            console.log("⚠️ Could not find transparent color button, trying alternative selectors...");
                            const colorBtns = document.querySelectorAll('button[id^="color-"]');
                            if (colorBtns.length > 0) {
                                colorBtns[0].click();
                                console.log("🎯 Clicked first available color button");
                            }
                        } else {
                            transBtn.click();
                        }
                        ...
                        console.log("🎯 Waiting for CAPTCHA challenge...");
                        await sleep(2000);
                        ...
                        const confirmLoop = async () => {
                            let attempts = 0;
                            while (!isTokenValid() && Date.now() - startTime < 25000) {
                                attempts++;
                                ...
                                if (confirmBtn && !confirmBtn.disabled) {
                                    console.log(`🎯 Clicking confirmation button (attempt ${attempts})...`);
                                    confirmBtn.click();
                                } else {
                                    console.log(`🎯 No active confirm button found (attempt ${attempts})`);
                                }
                        ...
                        await sleep(500);
                        console.log("✅ Token successfully captured through browser automation");
                        resolve(token);
                    })();
                    ...
                    console.log("❌ Auto-CAPTCHA process failed:", error);
                    reject(error);
                }
            };
    ...
    // WPLACE API SERVICE WITH ENHANCED PAWTECT + WASM PROTECTION
    const WPlaceService = {
        async paintPixelInRegion(regionX, regionY, pixelX, pixelY, color) {
            try {
                ...
                // Enhanced payload with fingerprint
                const payload = {
                    coords: [pixelX, pixelY],
                    colors: [color],
                    t: turnstileToken,
                    fp: randStr(10), // Random fingerprint like script.js
                };

                // Generate WASM token for x-pawtect-token header
                console.log('🔐 Generating WASM token for enhanced protection...');
                const wasmToken = await createWasmToken(regionX, regionY, payload);
                
                if (!wasmToken) {
                    console.warn('⚠️ WASM token generation failed, falling back to basic protection');
                }

                // Enhanced headers with x-pawtect-token
                const headers = {
                ...
                if (wasmToken) {
                    ...
                    console.log('🛡️ Using enhanced WASM protection');
                } else {
                    console.log('⚠️ Using basic protection only');
                }
