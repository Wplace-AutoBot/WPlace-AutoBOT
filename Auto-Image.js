;(async () => {
    ...
    const CONFIG = {
        ...
        RANDOMNESS: {
            ...
            BYTE_POOL_SIZE: 4096,
            API_URL: 'https://www.random.org/integers/?num={num}&min=0&max=255&col=1&base=10&format=plain&rnd=new'
        },
...
// Add this helper (place it after getCurrentTheme/switchTheme definitions)
    ...
    const Random = {
        ...
        async _refillPool(n) {
            try {
                ...
                // Keep pool null on failure; fallback per-byte
                this._pool = null;
                this._idx = 0;
                throw e;
            }
    ...
    `
        ...
        function showResizeDialog(processor) {
            ...
            maskCanvas.addEventListener('touchstart', (e) => { /* let panStage handle */
            }, {passive: true});
            maskCanvas.addEventListener('touchmove', (e) => { /* let panStage handle */
            }, {passive: true});
            maskCanvas.addEventListener('touchend', (e) => { /* let panStage handle */
            }, {passive: true});
            window.addEventListener('mousemove', (e) => {
    ...
    // Outline-first painting pipeline
    ...
    // Random-spots painting pipeline
    ...
    async function initializeTokenGenerator() {
        ...
        try {
            ...
            console.error("❌ Error during Turnstile initialization:", error);
            updateUI("tokenRetryLater", "warning");
            enableFileOperations();
        }
    }
    ...
    createUI().then(() => {
        ...
        const advancedInit = () => {
            ...
            if (chromaToggle) chromaToggle.addEventListener('change', e => {
                state.enableChromaPenalty = e.target.checked;
                saveBotSettings();
                _updateResizePreview();
            });
            if (chromaSlider && chromaValue) chromaSlider.addEventListener('input', e => {
                state.chromaPenaltyWeight = parseFloat(e.target.value) || 0.15;
                chromaValue.textContent = state.chromaPenaltyWeight.toFixed(2);
                saveBotSettings();
                _updateResizePreview();
            });
            if (transInput) transInput.addEventListener('change', e => {
            ...
            if (whiteInput) whiteInput.addEventListener('change', e => {
                const v = parseInt(e.target.value, 10);
