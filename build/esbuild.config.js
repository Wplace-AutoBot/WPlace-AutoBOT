...
const assetInjectPlugin = {
  ...
  setup(build) {
    ...
    build.onLoad({ filter: /.*/, namespace: 'asset-inject' }, () => {
      ...
      const mainCss = fs.readFileSync('./src/auto-image-styles.css', 'utf8')
        .replace(/\\/g, '\\\\')
        .replace(/`/g, '\\`')
        .replace(/\$\{/g, '\\${');
...
async function build(targetName = 'all') {
  ...
  if (targetName === 'all') {
    ...
    const bookmarkletContent = fs.readFileSync('./dist/auto-image.bookmarklet.js', 'utf8');
    const bookmarkletWrapped = `javascript:(function(){${bookmarkletContent}})();`;
    fs.writeFileSync('./dist/auto-image.bookmarklet.txt', bookmarkletWrapped);
